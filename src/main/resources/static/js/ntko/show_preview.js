...
function OnComplete3(str, doc) {
    var filePath = $("#tmpFilePath").val();
    var fileType = $("#fileType").val();
    filePath = encodeURIComponent(filePath);
    // 替换报价一览表和价格构成表
    // loadWord("BID_PRICE_TABLE_START", "BID_PRICE_TABLE_END", contextRoot + "/upload/readFile.html?filePath=" + filePath);
    var sel = OFFICE_CONTROL_OBJ.ActiveDocument.Application.Selection;
    if(OFFICE_CONTROL_OBJ.ActiveDocument.BookMarks.Exists("BID_PRICE_TABLE")){
        if(fileType == 1){
            OFFICE_CONTROL_OBJ.ActiveDocument.UnProtect("jdcg$810");
        }
        // 将光标定位到书签BID_PRICE_TABLE
        sel.GoTo(-1,0,0,"BID_PRICE_TABLE");
        try {
            OFFICE_CONTROL_OBJ.AddTemplateFromURL(contextRoot + "upload/readFile.html?filePath=" + filePath, false, true);
        } catch (e){
            alert("错误：" + err.number + ":" + err.description);
        }
    }else {
        alert("当前没有BID_PRICE_TABLE书签！");
    }
}
...
function show() {
    ...
    OFFICE_CONTROL_OBJ.SaveToLocal("D:\\投标文件【" + packageName + "】.doc", false, true);
...
function OnComplete2(type, code, html) {
    if (html != null && html != '') {
        ...
        if (params[0] == 200 && params[2] == 1) {
            // 进入预览页面
            window.location.href = contextRoot + "/home/show_preview/" + params[1];
        }
    }
}
